fn main() {
    ...
    if std::env::var("TARGET")
        ...
        .unwrap_or(false)
    {
        let android_assets_dir =
            std::path::PathBuf::from("gen/android/app/src/main/assets/engines");
        ...
        println!(
            "cargo:warning=Checking for bundled engines in: {}",
            source_dir.display()
        );

        if source_dir.exists() {
            println!(
                "cargo:warning=Source directory exists: {}",
                source_dir.display()
            );
            if let Err(e) = std::fs::create_dir_all(&android_assets_dir) {
                ...
                println!(
                    "cargo:warning=Android assets directory ready: {}",
                ...
                if let Ok(entries) = std::fs::read_dir(&source_dir) {
                    ...
                    for entry in entries.flatten() {
                        if entry.path().is_file() {
                            ...
                            if let Err(e) = std::fs::copy(entry.path(), &dest) {
                                ...
                                println!(
                                    "cargo:warning=Copied bundled engine: {} -> {}",
                                    entry.path().display(),
                                    dest.display()
                                );
                                copied_count += 1;
                    ...
                    if copied_count == 0 {
                        eprintln!(
                            "cargo:warning=No engine files found in {}",
                            source_dir.display()
                        );
                    }
                } else {
                    eprintln!(
                        "cargo:warning=Failed to read source directory: {}",
                        source_dir.display()
                    );
                }
            }
            ...
            if stockfish_src.is_file() {
                if let Err(e) = std::fs::create_dir_all(&android_jni_libs_dir) {
                    ...
                    match std::fs::copy(&stockfish_src, &stockfish_dst) {
                        ...
                        Err(e) => {
                            eprintln!("cargo:warning=Failed to copy Stockfish into jniLibs: {}", e)
                        }
                    }
                }
            } else {
            ...
            eprintln!(
                "cargo:warning=Bundled engines source directory does not exist: {}",
                source_dir.display()
            );
            eprintln!(
                "cargo:warning=Please ensure stockfish binary is placed at: {}",
                source_dir.display()
            );
        }
    }
