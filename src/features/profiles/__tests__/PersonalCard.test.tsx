...
describe("PersonalCard", () => {
  ...
  test("renders player name selector when showPlayerSelector is true", () => {
    ...
    const selectInput =
      screen.queryByDisplayValue("Test Player") || screen.queryByRole("combobox") || screen.queryByLabelText(/player/i);
  ...
  test("allows changing player name when setName is provided (best-effort)", async () => {
    ...
    const option = screen.queryByRole("option", { name: /player1/i }) ?? screen.queryByText(/player1/i);
