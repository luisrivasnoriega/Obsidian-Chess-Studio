...
/**
 * Generates PGN from the report view structure.
 * This follows the same logic as buildMainlineItems in GameNotation.tsx:
 * - Always follows children[0] as the main line
 * - Includes all variations as they appear in the report view
 * - Matches exactly what's displayed in the report view
 */
export function getPGNFromReportView(
  tree: TreeNode,
  {
    headers,
    glyphs,
  ...
  if (!currentNode.san) {
    if (currentNode.children?.length > 0) {
      ...
      // No moves at all - just return headers with result
      if (headers) {
        pgn += ` ${headers.result || "*"}`;
      } else {
        pgn += " *";
...
export function getGameStats(root: TreeNode) {
  ...
  while (node.children.length > 0) {
    ...
    for (const annotation of node.annotations) {
      if (isBasicAnnotation(annotation) || annotation === "BookError" || annotation === "BookUnknown") {
